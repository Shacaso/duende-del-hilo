...
export async function GET(req: Request, context: { params: any }) {
    ...
    const response: Costume | CustomError = await getById<Costume>(id, costumesPath)
...
export async function PATCH(req: Request, context: { params: any }) {
    ...
    const response: Costume | CustomError = await update<Costume>(id, body, costumesPath)
    if (response instanceof CustomError) return NextResponse.json(response, { status: response.codigo })
    return NextResponse.json(response)
}
...
export async function DELETE(req: Request, context: { params: any }) {
    const id = context.params.id
    const response: Costume | CustomError = await logicDelete<Costume>(id, costumesPath)
    if (response instanceof CustomError) return NextResponse.json(response, { status: response.codigo })
    return NextResponse.json(response)
}